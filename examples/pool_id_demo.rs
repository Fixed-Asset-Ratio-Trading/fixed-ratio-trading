...
fn main() {
    ...
    // Example 1: Using the utility function directly
    println!("\n📋 Method 1: Using derive_pool_id utility function");
    let (pool_id_1, bump_1) = derive_pool_id(
        &PROGRAM_ID,
        &usdc_mint,
        &sol_mint,
        1000,  // 1000 USDC per 1 SOL
        1,
    );
    println!("   Pool ID: {}", pool_id_1);
    println!("   Bump Seed: {}", bump_1);
    
    // Example 2: Using the client SDK
    println!("\n📋 Method 2: Using PoolClient");
    ...
    let pool_id_2 = pool_client.derive_pool_id(&config);
    println!("   Pool ID: {}", pool_id_2);
    
    // Verify both methods give the same result
    assert_eq!(pool_id_1, pool_id_2);
    println!("   ✅ Both methods produce the same Pool ID!");
    
    // Example 3: Demonstrate normalization (order doesn't matter)
    println!("\n📋 Method 3: Demonstrating token order normalization");
    ...
    let pool_id_3 = pool_client.derive_pool_id(&config_swapped);
    println!("   Pool ID (swapped order): {}", pool_id_3);
    println!("   ✅ Same Pool ID regardless of token parameter order!");
    
    // Example 4: Different ratios produce different pools
    println!("\n📋 Method 4: Different ratios = different pools");
    ...
    let pool_id_4 = pool_client.derive_pool_id(&config_different_ratio);
    println!("   Pool ID (2000:1 ratio): {}", pool_id_4);
    assert_ne!(pool_id_1, pool_id_4);
