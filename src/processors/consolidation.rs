...
use crate::{
    ...
    utils::validation::validate_and_deserialize_pool_state_legacy,
};
...
fn perform_batch_consolidation(
    ...
    for pool_account in pool_accounts {
        let mut pool_state = validate_and_deserialize_pool_state_legacy(pool_account, program_id)?;
...
pub fn get_consolidation_status(
    ...
    for (i, pool_account) in pool_accounts.iter().enumerate() {
        let pool_state = validate_and_deserialize_pool_state_legacy(pool_account, program_id)?;
