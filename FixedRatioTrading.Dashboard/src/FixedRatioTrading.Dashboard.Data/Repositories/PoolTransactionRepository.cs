...
public class PoolTransactionRepository : IPoolTransactionRepository
{
    ...
    public async Task<(ulong TokenAVolume, ulong TokenBVolume)> GetSwapVolumeAsync(Guid? poolId = null, TimeSpan? period = null) { var txs = await _context.PoolTransactions.Where(t => t.Type == TransactionType.Swap).ToListAsync(); return (txs.Sum(t => t.TokenAAmount), txs.Sum(t => t.TokenBAmount)); }
